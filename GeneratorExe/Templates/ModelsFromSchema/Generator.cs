...
namespace CodeGeneration.ModelsFromSchema {
  ...
  public class Generator {
    ...
    public void Generate(CodeWriterBase writer, Cfg cfg) {
      ...
      string rawSchema = File.ReadAllText(cfg.inputFile, System.Text.Encoding.Default);
    ...
    // writer.Write(schema.SemanticVersion);

      foreach (var entity in schema.Entities) {
        writer.BeginClass(AccessModifier.Public, entity.Name, null, true);
        writer.WriteLine();
        ...
        foreach (var field in entity.Fields ) {
         ...
         // annotations
          ...
          if (CodeWriterBase.TryResolveToCommonType(typeName, ref ct)) {
            typeName = writer.GetCommonTypeName(ct); //in target language
            defaultValue = writer.GetContantValue(ct, field.DefaultValue,true);
          }



          writer.Summary(field.Summary,false);
          writer.InlineProperty(AccessModifier.Public, field.Name, typeName, defaultValue);
          writer.WriteLine();
        }
