...
$ttIncludeFile = join-path $deployTarget 'kGEN.ttinclude'
...
ls $deployTarget | where-object { $_.Name -eq "kGEN.Readme.txt" } | foreach-object { 
    $folderItems.AddFromFile($_.FullName) > $null
} > $null

ls $deployTarget | where-object { $_.Name -eq "kGEN.Sample.tt" } | foreach-object { 
    $folderItems.AddFromFile($_.FullName) > $null
} > $null
